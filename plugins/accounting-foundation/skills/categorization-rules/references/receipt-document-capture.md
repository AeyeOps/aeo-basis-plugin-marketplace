...
- [Accounting System Receipt Capture](#accounting-system-receipt-capture)
...
## Accounting System Receipt Capture

### Upload Methods

- **Mobile app** — photograph receipt directly in the accounting system's mobile app; triggers OCR immediately
- **Email forwarding** — forward receipt emails to the company's designated receipt ingestion address
- **Web upload** — drag image or PDF files onto the receipts interface in the browser
- **Bank feed attachment** — open a matched transaction and attach the receipt file manually

For platform-specific setup steps and navigation, invoke `qbo-integration:qbo-bookkeeping` or the equivalent integration skill.

### OCR and Matching

Built-in OCR extracts vendor name, date, amount, and payment method from the receipt image, then suggests a match against existing bank feed transactions using amount and date proximity.

- **Auto-matched**: OCR amount matches a bank feed transaction within a date window — review and confirm
- **Unmatched**: no bank feed match found — the system creates a draft expense that the bookkeeper categorizes manually
...
- File size and format limits vary by platform (common: 20 MB, JPEG/PNG/GIF/PDF)
...
Several dedicated receipt capture tools integrate with major accounting systems. Key capabilities to evaluate when selecting a tool:

- **Bulk capture** — mobile, email forwarding, or auto-fetch from supplier portals
- **Multi-currency support** — automatic conversion at transaction date rates
- **Accounting system sync** — pushes categorized expenses as draft transactions
- **Line-item extraction** — itemized receipt detail, not just totals
- **Auto-fetch** — recurring documents pulled from banks, utilities, and suppliers automatically
- **SMS/text capture** — for clients who resist app-based workflows

Selection criteria: firms managing multiple clients or high receipt volumes (50+/month per client) benefit from a dedicated third-party tool. Lower volumes can use the accounting system's native receipt capture.
...
- **Set up email forwarding** — create a rule in the client's email to auto-forward receipts from common vendors to the accounting system's receipt ingestion address
...
- Third-party tool vs. native capture — use third-party tools when managing 3+ clients or when receipt volume exceeds 50/month per client; the accounting system's native capture is sufficient for lower volumes
...
- For platform-specific receipt capture workflows, invoke `qbo-integration:qbo-bookkeeping` or the equivalent integration skill
