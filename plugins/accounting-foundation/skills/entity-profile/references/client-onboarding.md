...
Execute before any work. Must specify: scope (bookkeeping only vs bookkeeping + tax), deliverable frequency, client responsibilities (document delivery, response time), fee structure and payment terms, period covered, software responsibility (firm or client pays for accounting system), and termination provisions.
...
**Payroll**: provider (ADP/Gusto/Paychex/in-platform payroll/manual), employee count (W-2) and contractor count (1099), pay frequency, states where employees work

**Tax/compliance**: sales tax permit numbers and registered states, prior IRS/state notices, pending audits

## Company Setup in the Accounting System
...
For platform-specific setup steps, delegate to `qbo-integration:qbo-coa`.
...
2. Start from firm's entity-type template (invoke `accounting-foundation:chart-of-accounts` for templates)
...
For platform-specific COA configuration, delegate to `qbo-integration:qbo-coa`.

## Opening Balance Entry

**Approach A — Opening Balance Equity** (simplest): enter balances directly in each account; the system offsets to OBE; reclassify OBE to Retained Earnings after all entered. Best for known trial balance from tax return.
...
**Approach C — Import/Migration**: use the accounting system's import tools or migration from prior software. Verify imported balances against source trial balance.

**Source**: prior year Schedule L (corps) or accountant's year-end trial balance.

## Bank and Credit Card Connections

1. Link each account in the accounting system's banking module (client connects via invite flow or shares credentials)
...
For platform-specific user management, delegate to `qbo-integration:qbo-coa`.
...
Establish: primary contact (name, email, phone), response time expectation (recommend 3 business days), document delivery method (shared folder or receipt capture tool), question protocol (batch weekly), monthly review meeting (first 3 months, then quarterly), escalation path if primary is unresponsive.
...
For platform-specific procedures (accountant toolbox, invitation flow, data import, app integrations, bank feed authentication), delegate to `qbo-integration:qbo-coa` or the relevant platform integration skill.
...
- Cash basis books but accrual returns — clarify with CPA; set accounting system to match tax return method
- Mixed personal/business transactions — separate during onboarding; set clear policy; use owner draws/contributions
- Multiple entities, one owner — separate company file per entity; intercompany accounts for transfers

## References

- AICPA SSARS — engagement letter requirements
- For platform-specific setup and import documentation, see `qbo-integration` plugin
