...
- [Accounting System Integration Notes](#accounting-system-integration-notes)
...
- **Posting to locked periods** — if the accounting system lock date is not set, late entries slip into prior months and invalidate delivered financials
...
- **Bank feed over-reliance** — auto-matching in the accounting system can create duplicates or miscategorize; always review matches manually
...
For platform-specific close procedures (lock dates, reconciliation tools, recurring transactions, bank feeds, report generation, journal entry tagging, and class/location tracking), invoke `qbo-integration:qbo-reporting` or the applicable platform integration skill. The steps below are system-agnostic principles:

- **Lock Date**: Set both the closing date and password protection. Support separate lock dates for accountant vs client where available.
- **Reconciliation**: Track reconciliation status per account; retain completed reconciliation history for audit trail.
- **Recurring Transactions**: Schedule monthly adjusting entries (depreciation, prepaids) to auto-post.
- **Bank Feeds**: Review auto-matched transactions daily during the month to reduce close burden. Use categorization rules to auto-classify high-frequency vendors.
- **Reports**: Generate P&L, Balance Sheet, and Trial Balance with period comparison. Match report period to the close month exactly.
- **Journal Entries**: Use the memo field to tag close-related JEs (e.g., "Month-end close — Dec 2025 depreciation") for audit trail.
- **Class/Location Tracking**: If enabled, verify all transactions are tagged before close; unclassified transactions distort segment reporting.
...
- Intuit. "Month-End Close." (reference)
