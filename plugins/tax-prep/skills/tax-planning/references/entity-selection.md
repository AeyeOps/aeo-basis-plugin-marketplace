...
For platform-specific setup procedures, invoke `qbo-integration:qbo-coa` for chart of accounts configuration and `qbo-integration:qbo-bookkeeping` for entity setup.
...
- Accounting system company setup: select the correct entity type (Corporation, S-Corp, Partnership, Sole Proprietor) during initial configuration. This affects tax mapping and default account categories.
- S-corp shareholder basis tracking is typically not handled natively in the accounting system. Maintain a separate basis schedule outside the GL.
